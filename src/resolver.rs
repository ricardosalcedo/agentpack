...
pub fn resolve(manifest: &Manifest) -> Result<LockFile> {
    ...
    Ok(LockFile { lock_version: 1, resolved })
}
...
fn resolve_mcp(name: &str, version_req: &str) -> Result<ResolvedEntry> {
    ...
    if Path::new(&local_path).exists() {
        ...
        return Ok(ResolvedEntry {
            ...
            source: Source { source_type: "local".into(), package: Some(local_path), version: Some(m.version) },
...
fn resolve_agent(name: &str, dep: &AgentDep) -> Result<ResolvedEntry> {
    ...
    if Path::new(&local_path).exists() {
        ...
        let agent_deps: BTreeMap<String, String> = m.agents.iter()
    ...
    Ok(ResolvedEntry {
        version: resolved_version,
        entry_type: "agent".into(),
        source: Source { source_type: source_url.into(), package: Some(name.into()), version: Some(dep.version().into()) },
        integrity: Some(integrity),
        transport: None,
        dependencies: BTreeMap::new(),
        agents: BTreeMap::new(),
...
fn infer_source(name: &str, version: &str) -> (Source, Transport) {
    let package_name = name.strip_prefix("io.github.").unwrap_or(name).replace('/', "-");
    ...
    (
        Source { source_type: "npm".into(), package: Some(npx_package.clone()), version: Some(version.into()) },
        Transport { transport_type: "stdio".into(), command: Some("npx".into()), args: vec!["-y".into(), npx_package], url: None },
    )
}

...
pub fn run() -> Result<()> {
    ...
    for (name, dep) in &manifest.agents {
        ...
        if !Path::new(&local_path).exists() {
            issues.push(format!(
                "[WARN] Cannot validate '{}': no local manifest. Run `agentpack fetch` first.",
                name
            ));
            continue;
        }
        ...
        let provided: Vec<&str> = agent_manifest
            .provides
            .as_ref()
    ...
    if issues.is_empty() {
        println!(
            "✓ All agent capabilities validated. {} agent deps checked.",
            manifest.agents.len()
        );
    } else {
